...
class MoveToPose(Utility):
    ...
    def __init__(self):
        ...
        self.is_behind_robot_      = False

        # Create action server:
        self.as_ = actionlib.SimpleActionServer("move_to_pose", MoveToPoseAction, execute_cb=self.executeCB, auto_start=False)
        self.as_.start()
    ...
    def calStartAndGoal(self, start_frame_name:str, goal_frame_name:str):
        ...
        if (abs(goal_direction) > math.pi/2):
            self.is_behind_robot_ = True
        else:
            self.is_behind_robot_ = False
        
        if (self.is_behind_robot_):
